...
│       ├── constants.js           # 统一的浏览器 User-Agent
│       └── providers/
...
- 🐛 **修复豆瓣抓取静默丢数据**：`extractInPage` 及其所有辅助函数改为内联在单一自包含函数内，修复 `page.evaluate` 不捕获 Node 模块作用域导致的 `ReferenceError`（刷新时电影/书/专辑会被丢弃）。
- 🐛 **修复 Steam 重复抓取**：`refresh.js` 的 `steam` 必需字段去掉 `localPosterPath`（部分 app 的 `library_600x900.jpg` 不存在，会导致永远不满足刷新条件、每次都重抓），改为 `['posterUrl', 'localCoverPath']`。
- ♻️ `collections.ts`：移除 `normalize()` 中永不触发的 `{ items: [...] }` 死分支。
- ♻️ `index.astro`：为内容查看器补充初始 `data-active-view="games"`，明确首屏默认标签页。
- 📝 重写 `README-Logbook.md`：新增「维护者须知（易踩的坑）」与更准确的 Agent 指南；补充 `npm run check`、`node --check` 校验说明。
- ✨ 调整项目定位为「灵魂旅途日志」：明确它不是简历式个人主页，而是记录真正影响过自己的书、电影、游戏、音乐——「我为什么会成为现在的我」。

### 更早

- 重写 `README-Logbook.md` 使其与真实代码一致（脚本拆分到 `scripts/lib`、主题色提取为 canvas 平均色、动态路由 `[collection].astro`、支持平台不含 Epic 等）。
- 修复箭头按钮滚动逻辑（`getBoundingClientRect` 精确计算）；统一内容面板宽度；提高正文对比度；`.glass-panel` 工具类化；主题色自动提取；支持 Steam / 豆瓣 / MusicBrainz 数据源。
